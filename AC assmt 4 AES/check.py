...
def encrypt(plaintext, key, rounds):
    ...
    for round in range(1, rounds + 1):
        ...
        if round < rounds:
            ...
            # print(state)
            state1 = [state[j][i] for i in range(4) for j in range(4)]
            # print(state1)
            # state = state1
            state = [[state1[i+4*j] for i in range(4)] for j in range(4)]
...
def main():
    ...
    # Convert plaintext to a 4x4 matrix for rearrangement
    key_matrix = [[key[i + 4 * j] for i in range(4)] for j in range(4)]

    # Rearrange plaintext_matrix into a column-major list
    key = [key_matrix[j][i] for i in range(4) for j in range(4)]
    # print(key)
